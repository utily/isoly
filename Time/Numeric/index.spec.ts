...
describe("isoly.Time.Numeric", () => {
	...
	it.each([
		...
		[3723004, { hours: 1, minutes: 2, seconds: 3, milliseconds: 4 }],
		[86399999, { hours: 23, minutes: 59, seconds: 59, milliseconds: 999 }],
		[0, { hours: 0, minutes: 0, seconds: 0, milliseconds: 0 }],
		[32887006, { hours: 9, minutes: 8, seconds: 7, milliseconds: 6 }],
		[56730123, { hours: 15, minutes: 45, seconds: 30, milliseconds: 123 }],
		[45296000, { hours: 12, minutes: 34, seconds: 56, milliseconds: 0 }],
		[45240000, { hours: 12, minutes: 34, seconds: 0, milliseconds: 0 }],
		[43200000, { hours: 12, minutes: 0, seconds: 0, milliseconds: 0 }],
		[86400000, { hours: 24, minutes: 0, seconds: 0, milliseconds: 0 }],
		[489556789, { hours: 135, minutes: 59, seconds: 16, milliseconds: 789 }],
		[230443211, { hours: 64, minutes: 0, seconds: 43, milliseconds: 211 }],
	])('create(%d, "milliseconds") == %s', (epoch, expected) =>
		expect(isoly.Time.Numeric.create(epoch, "milliseconds").value).toEqual(expected)
	)
	it.each([
		[
		...
		[{ minutes: 123 }, { hours: 2, minutes: 3 }],
		[{ seconds: 86400 }, { hours: 24, minutes: 0, seconds: 0 }],
		[
			{ hours: 12, minutes: 34 },
			{ hours: 12, minutes: 34 },
		],
	...
	])("normalize(%s) == %s", (time, expected) =>
		expect(isoly.Time.Numeric.from(time).normalize().value).toEqual(expected)
	)
	it.each([
	...
	] as const)("format(%s) == %s", (time, expected) => expect(isoly.Time.Numeric.from(time).format()).toEqual(expected))
	it.each([
		...
		expect(isoly.Time.Numeric.from(value).truncate(precision).value).toEqual(expected)
	)
	it.each([
		[{ hours: 12 }, "hours"],
	...
	] as const)("precision(%s) == %s", (value, expected) =>
		expect(isoly.Time.Numeric.from(value).precision).toEqual(expected)
	)
})
